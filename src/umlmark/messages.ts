...
export class Messages {
    ...
    static async showUpdateMessage(version: string): Promise<string | undefined> {
        ...
        if (result === viewReleaseNotes) {
            commands.executeCommand("vscode.open", Uri.parse('https://marketplace.visualstudio.com/items/petercai.umlmark/changelog'));
        } else if (result === viewDocs) {
            commands.executeCommand("vscode.open", Uri.parse('https://marketplace.visualstudio.com/items/petercai.umlmark'));
        }
        return result;
    }
